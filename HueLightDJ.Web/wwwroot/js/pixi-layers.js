    ...
    var Group = (function(_super) {
        ...
        Group.conflict = function() {
            if (Group._lastLayerConflict + 5000 < Date.now()) {
                ...
                console.log(
                    "PIXI-display plugin found two layers with the same group in one stage - that's not healthy. Please place a breakpoint here and debug it");
            }
        };
            ...
            _displayProcessInteractive: function(point, displayObject, hitTestOrder, interactive, outOfMask) {
                ...
                if (displayObject.interactiveChildren && children) {
                    for (var i = children.length - 1; i >= 0; i--) {
                        ...
                        var hitChild =
                            this._displayProcessInteractive(point, child, hitTestOrder, interactiveParent, outOfMask);
                        if (hitChild) {
                            if (!child.parent) {
                                continue;
                            }
